...
namespace DotNetCoreKoans.Koans
{
    public class AboutGenericContainers : Koan
	{
		...
		public void ArrayListHoldsObjects()
		{
			...
			//this assertion looks at the method defined above as Add (on list) and looks for its parameters, which are going to be objects, because objects are being added to the array list
		...
		public void ArrayListIsNotStronglyTyped()
		{
			...
			//ArrayList can hold more than one type. (IMPLICIT IN THE FACT THAT THEY HOLD OBJECTS, YES?)
			ArrayList list = new ArrayList();
			list.Add(42);
			list.Add("fourty two");
		...
		public void InsertingInTheMiddle()
		{
			List<int> list = new List<int> { 1, 2, 3 };
			list.Insert(1, 6);
			//inserts INTO position [1], not after
		...
		public void RemovingItems()
		{
			...
			list.Remove(2);
			//removes the first occurence of that object (not index value!!!!)
			Assert.Equal(new int[] {1, 2, 3}, list.ToArray());
		}
		[Step(15)]
